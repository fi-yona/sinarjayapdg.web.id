...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($tokenResult->num_rows > 0) {
        $daftarPesananQuery = "SELECT
                                ...
                                WHERE
                                    tb_pesanan.username = '$username' AND tb_pesanan.tanggal_pesanan = '$tanggal'
                                ORDER BY
                                    tb_pesanan.waktu_pesanan DESC";
        $daftarPesananResult = $conn->query($daftarPesananQuery);

        $rows = array();
        while ($row = $daftarPesananResult->fetch_assoc()) {
            $rows[] = $row;
        }
